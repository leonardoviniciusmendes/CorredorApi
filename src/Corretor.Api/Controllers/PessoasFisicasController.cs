...
public sealed class PessoasFisicasController(CorretorDbContext db) : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<PessoaFisicaResponse>>> Get(CancellationToken cancellationToken)
    {
        var pessoas = await db.PessoasFisicas.AsNoTracking()
            .Select(x => new PessoaFisicaResponse(x.Id, x.Nome, x.Cpf, x.Email, x.Telefone, x.FaixaEtaria))
    ...
    public async Task<ActionResult<PessoaFisicaResponse>> Post(PessoaFisicaRequest request, CancellationToken cancellationToken)
    {
        ...
        var response = new PessoaFisicaResponse(pessoa.Id, pessoa.Nome, pessoa.Cpf, pessoa.Email, pessoa.Telefone, pessoa.FaixaEtaria);
...
public sealed record PessoaFisicaRequest([Required] string Nome, [Required] string Cpf, string? Email, string? Telefone, string? FaixaEtaria);
public sealed record PessoaFisicaResponse(Guid Id, string Nome, string Cpf, string? Email, string? Telefone, string? FaixaEtaria);
